...
int main()
{
	...
	while (!end)
	{
		printf("dogHouse=%p\tdogPtr=%p\n%d\n", house_ptr, dog_ptr, house_ptr - dog_ptr);
		...
		case '4': // WATCH HIS DEATH
			...
			dog->death(dog);
			break;
		case '5': // BUILD DOG HOUSE
			dogHouse = newDogHouse();
			break;
		case '6': // GIVE DOG HOUSE TO YOUR DOG
			if (!dog)
			{
				puts("You do not have a dog.");
				break;
